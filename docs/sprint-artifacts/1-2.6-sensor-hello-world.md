...
- [ ] Task 1: Create sensor_hello_world.py CLI framework (AC: 1)
  - [ ] 1.1 Set up argparse with flags: --camera, --lidar, --audio, --rviz, --all
  - [ ] 1.2 Add network interface argument (required, like existing scripts)
  - [ ] 1.3 Add --output-dir argument for saving captured data (default: ./sensor_captures/)
  - [ ] 1.4 Add --duration argument for audio recording length (default: 5 seconds)
  - [ ] 1.5 Add --lidar-duration argument for point cloud accumulation (default: 2 seconds)
  - [ ] 1.6 Implement safety banner and confirmation prompt (reuse pattern from hello_world_g1.py)
  - [ ] 1.7 Structure code with SensorTester class for each sensor type

- [ ] Task 2: Implement camera capture via pyrealsense2 (AC: 2)
  - [ ] 2.1 Initialize RealSense D435 via pyrealsense2 (direct SDK - no ROS2 dependency)
  - [ ] 2.2 Capture single RGB frame and save as PNG
  - [ ] 2.3 Capture aligned depth frame and save as PNG (16-bit) and numpy .npy
  - [ ] 2.4 Display image dimensions, depth range, and file paths
  - [ ] 2.5 Handle camera not found error gracefully with clear message

- [ ] Task 3: Implement LiDAR capture via ROS2 topic (AC: 3)
  - [ ] 3.1 Subscribe to `utlidar/cloud` ROS2 topic (Unitree built-in, no driver setup needed)
  - [ ] 3.2 Accumulate point cloud for configurable duration (default 2 seconds, use --lidar-duration)
  - [ ] 3.3 Save point cloud to PCD file using open3d
  - [ ] 3.4 Display point count, bounding box dimensions, and file path
  - [ ] 3.5 Handle timeout if no LiDAR data received (5 second timeout)

- [ ] Task 4: Implement audio capture via sounddevice (AC: 4)
  - [ ] 4.1 Discover audio devices via sounddevice.query_devices(), find 4+ channel device
  - [ ] 4.2 Record audio for specified duration (default 5 seconds via --duration)
  - [ ] 4.3 Save to WAV file with proper sample rate (48000 Hz typical)
  - [ ] 4.4 Display duration, sample rate, channels, and file path
  - [ ] 4.5 Handle microphone not found error gracefully with device list

- [ ] Task 5: Implement RViz visualization (AC: 5)
  - [ ] 5.1 Create RViz config file at `src/g1_bringup/config/rviz/sensor_test.rviz`
  - [ ] 5.2 Add PointCloud2 display for LiDAR (topic: `utlidar/cloud`, frame: `utlidar_lidar`)
  - [ ] 5.3 Add Image display for camera RGB (topic from realsense driver)
  - [ ] 5.4 Add Image display for camera depth (topic from realsense driver)
  - [ ] 5.5 Launch RViz when --rviz flag is provided
  - [ ] 5.6 Keep RViz running until user presses Ctrl+C

- [ ] Task 6: Integration and testing (AC: 1-6)
  - [ ] 6.1 Test --camera flag captures and saves images
  - [ ] 6.2 Test --lidar flag captures and saves point cloud
  - [ ] 6.3 Test --audio flag records and saves audio
  - [ ] 6.4 Test --all flag runs all sensors sequentially
  - [ ] 6.5 Test --rviz flag launches visualization with point cloud
  - [ ] 6.6 Test --all --rviz combination shows visualization during full sensor test
  - [ ] 6.7 Verify all files have timestamps in filenames
  - [ ] 6.8 Add unit tests at `scripts/test_sensor_hello_world.py` using pytest
  - [ ] 6.9 Add `sensor_captures/` to `.gitignore`
